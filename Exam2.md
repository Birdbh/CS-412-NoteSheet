...
*   **Clustering Entropy $H(C)$:** Represents the entropy of the output clusters; calculated as $- \sum (P_{C_i} \log P_{C_i})$ where $P_{C_i}$ is the probability of a point being in cluster $i$ (Cluster Row Sum / Total $N$).
*   **Ground Truth Entropy $H(T)$:** Represents the entropy of the actual classes; calculated as $- \sum (P_{T_j} \log P_{T_j})$ where $P_{T_j}$ is the probability of a point being in class $j$ (Ground Truth Column Sum / Total $N$).
*   **Conditional Entropy $H(T|C)$:** Represents the entropy of the ground truth given the clustering; first calculate each row's individual entropy $H(T|C_i) = - \sum (p \log p)$ where $p$ is (Cell Value / Cluster Row Sum), then calculate the final value by taking the weighted average of all rows $\sum (\frac{\text{Cluster Row Sum}}{N} \times H(T|C_i))$.
