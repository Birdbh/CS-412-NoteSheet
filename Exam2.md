...
*   **Entropy:** For each row, calculate $e_i = - \sum (p \log p)$ where $p$ is (cell value / row sum), then calculate total conditional entropy by taking the weighted average of all $e_i$ values based on row sizes $\sum \left(\frac{\text{row\ sum}}{N} \times e_i\right)$
