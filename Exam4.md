...
sequenceDiagram
    ...
    Note over Client,Server: Server acknowledges and opens its own channel
...
graph TD
    ...
    Note["A calculates path to D:<br>Via B: 1+2 = 3 ✓<br>Via C: 3+5 = 8<br>Chooses path via B"]
    A -.-> Note
```
...
graph TD
    ...
    Overflow["Buffer overflows on simultaneous<br>fan-in — packets drop, TCP timeouts"]
    Switch -.-> Overflow

    Switch -- "3. Bottlenecked delivery" --> Agg
```
...
graph TD
    ...
    C -- "OpenFlow / API — push forwarding rules" --> S1
    C -- "OpenFlow / API" --> S2
    C -- "OpenFlow / API" --> S3
```
...
```mermaid
graph TD
    Start[Packet arrives at vSwitch] --> CheckCache{Match in kernel cache?}

    CheckCache -- "Yes — fast path" --> Forward[Forward packet]

    CheckCache -- "No — slow path" --> Userspace[Send to userspace pipeline]
    Userspace --> Compute[Compute action]
    Compute --> UpdateCache[Install rule in kernel cache]
    UpdateCache --> Forward
```
